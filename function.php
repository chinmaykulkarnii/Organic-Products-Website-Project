	...
	<div class="'.(empty($opts['class']) ? 'notice notice-success' : 'popularfx-templates-promo').'" id="popularfx_templates_promo" style="min-height:90px">
		...
		<a class="popularfx_promo-close" href="javascript:" aria-label="Dismiss this Notice">
			<span class="dashicons dashicons-dismiss"></span> Dismiss
				...
				<br>
				'.(empty($opts['pro_url']) || !empty($popularfx['license']) ? '' : '<a class="button button-primary" target="blank" href="'.esc_url($opts['pro_url']).'">'._('Buy PopularFX Pro', 'popularfx').'</a>').'
				'.(empty($opts['rating']) ? '' : '<a class="button button-primary" target="blank" href="'.esc_url($opts['rating']).'">'._('Rate it 5★\'s', 'popularfx').'</a>').'
				'.(empty($opts['facebook']) ? '' : '<a class="button button-secondary" target="blank" href="'.esc_url($opts['facebook']).'"><span class="dashicons dashicons-thumbs-up" style="vertical-align: middle;"></span> '._('Facebook', 'popularfx').'</a>').'
				'.(empty($opts['twitter']) ? '' : '<a class="button button-secondary" target="blank" href="'.esc_url($opts['twitter']).'"><span class="dashicons dashicons-twitter" style="vertical-align: middle;"></span> '._('Tweet', 'popularfx').'</a>').'
				'.(empty($opts['website']) ? '' : '<a class="button button-secondary" target="blank" href="'.esc_url($opts['website']).'">'._('Visit our website', 'popularfx').'</a>').'
			</p>
		</div>
	</div>';
 ...
 */
require dirname( _FILE_ ) . '/inc/template-tags.php';

/**
 * Functions which enhance the theme by hooking into WordPress.
 */
require dirname( _FILE_ ) . '/inc/template-functions.php';

/**
 * Customizer additions.
 */
require dirname( _FILE_ ) . '/inc/customizer.php';

/**
 * Load WooCommerce compatibility file.
 */
if ( class_exists( 'WooCommerce' ) ) {
	require dirname( _FILE_ ) . '/inc/woocommerce.php';
}

// Update the theme
require_once dirname( _FILE_ ) . '/inc/popularfx.php';